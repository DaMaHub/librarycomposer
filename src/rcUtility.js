...
class RCutility extends events.EventEmitter {
  ...
  *
  */
  expMatchModuleGenesis(allRefcont, expModules) {
    let expHolder = []
  ...
  *
  */
  expMatchModuleJoined(allRefcont, expModules) {
    let expJOINHolder = []
  ...
  *
  */
  expMatchModuleLive(allRefcont, expModules) {
    let expHolder = []
    for (const exp of expModules) {
      // match this key to module
      for (const mrf of allRefcont) {
  ...
  *
  */
  extractQuestion(modules, typeAsk) {
    let question = ''
  ...
  *
  */
  extractCompute(modules, typeAsk) {
    let computes = []
    for (const mod of modules) {
      if (mod.value.info.moduleinfo.name === typeAsk) {
  ...
  *
  */
  extractVisualise(modules, typeAsk) {
    let visuals = []
    for (const mod of modules) {
      if (mod.value.info.moduleinfo.name === typeAsk) {
  ...
  *
  */
  extractEducation(modules, typeAsk) {
    let education = []
    for (const mod of modules) {
      if (mod.value.info.moduleinfo.name === typeAsk) {
  ...
  *
  */
  extractQuestionJOINED(modules, typeAsk) {
    let question = ''
    for (const mod of modules) {
  ...
  *
  */
  extractRefcontracts(modulesGrid, peerContext) {
    let newDisplayOrder = []
