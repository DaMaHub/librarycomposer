'use strict'
/**
*  Prepare Question Reference Contracts
*
*
* @class QuestionReferenceContract
* @package    HOP
* @copyright  Copyright (c) 2024 James Littlejohn
* @license    http://www.gnu.org/licenses/old-licenses/gpl-3.0.html
* @version    $Id$
*/
import { EventEmitter } from 'events';
import { validateContract } from '../validation/validationUtility.js';

class QuestionReferenceContract extends EventEmitter {
  constructor(heliLive) {
    super();
    this.heliLive = heliLive;
  }

  /**
  * prepare a Question reference contract
  * @method questionPrepare
  *
  */
  questionPrepare(inputRC) {
    const currentTime = this.heliLive ? this.heliLive.helistamp() : Date.now();
    const questionContract = {
      refcontract: 'question',
      concept: inputRC,
      computational: {},
      space: { concept: 'mind' },
      time: {
        createTimestamp: currentTime,
        lastTimestamp: currentTime,
        frequencyCount: 0
      }
    };
    
    return validateContract('question', questionContract);
  }
}

export default QuestionReferenceContract;
